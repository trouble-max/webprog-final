...
if (isset($_POST["submit"])) {
    $card = CreditCard::validCreditCard($_POST["card"]);
    if (!$card["valid"]) {
        header("location:../frontend/error.php?err=invalid%20card");
    }
    $card = $_POST["card"];
    ...
    $query = mysqli_query($mysqli, "SELECT * FROM `user` WHERE `card_number` = $user_card AND `balance` >= $sum");
    $check = mysqli_fetch_object($query);
    if (!$check) {
        header("location:../frontend/error.php?err=not%20enough%20money");
        exit;
    }
    $user_balance = $check->balance;
    echo $user_balance;
    
    $query = mysqli_query($mysqli, "SELECT * FROM `user` WHERE `card_number` = '$card'");
    $receiver = mysqli_fetch_object($query);
    if (!$receiver) {
        header("location:../frontend/sending_money.php?sum=".$sum.",".$user_balance);
        exit;
    }
    $receiver_id = $receiver->id;
    $receiver_balance = $receiver->balance;

    $new_user_balance = $user_balance - $sum;
    ...
    $query = mysqli_query($mysqli, "UPDATE `user` SET `balance` = '$new_receiver_balance' WHERE `user`.`id` = $receiver_id");
